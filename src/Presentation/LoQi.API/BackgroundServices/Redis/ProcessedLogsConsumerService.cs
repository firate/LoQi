...
public class ProcessedLogsConsumerService : BackgroundService
{
    ...
    {
        _logger.LogInformation("ProcessedLogsConsumerService started with consumer name: {ConsumerName}", _consumerName);
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                ...
                if (messages.Length > 0)
                {
                    ...
                    // await logService.BulkInsertLogsAsync(logMessages);
