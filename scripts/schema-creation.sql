...
PRAGMA
synchronous=NORMAL;  -- Instead of FULL (default)

-- Increase cache size (100MB cache)
PRAGMA
cache_size=100000;   -- 100K pages * 1KB = ~100MB
...
PRAGMA
page_size=4096;  -- 4KB pages (good for text data)
...
CREATE INDEX IF NOT EXISTS idx_logs_level_timestamp ON logs(level, timestamp DESC) WHERE level >= 3; --Warning and upper levels
...
CREATE
VIRTUAL TABLE IF NOT EXISTS logs_fts USING fts5(
    message,
    content='logs',
    content_rowid='id',
    -- Finds "connect", "connecting", "connected", "connection"
...
SELECT 'SQLite Version' as setting,
